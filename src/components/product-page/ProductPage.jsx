...
const ProductPage = () => {
  ...
  return (
    <div className="productPage content">
      <SEO
        title={`${import.meta.env.VITE_WEBSITE_NAME} ${productTitle}`}
        description={`${import.meta.env.VITE_WEBSITE_NAME} ${productTitle}`}
              ...
              <Grid item sm={6} xs={12} className="mainProductPhotosContainer">
                ...
                <Slider {...settings}>
                  {allImgHrefs?.map((myImage, index) => (
                    <img
                      key={`${myImage}_${index}`}
                      src={`${import.meta.env.VITE_IMAGES_URL}/${myImage}`}
                      alt={productTitle}
                      title={productTitle}
                    />
                  ))}
